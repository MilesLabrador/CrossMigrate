...
export const PALETTE = [
  ...
  },
  {
    ...
    items: [
      ...
      { type: 'convertTypes', label: 'Convert Types', icon: Binary,        desc: 'Cast fields with error handling' },
      { type: 'deduplicate',   label: 'Deduplicate',     icon: CopyMinus, desc: 'Remove duplicate rows' },
      { type: 'join',          label: 'Join',            icon: Merge,     desc: 'Join two tables & check key matches' },
      { type: 'lookup',        label: 'Lookup',          icon: SearchCheck, desc: 'Map values from a typed or cached table' },
      { type: 'cache',         label: 'Cache',           icon: Archive,     desc: 'Cache rows — skip refetching sources' },
